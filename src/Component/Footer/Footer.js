...
function Footer() {
  const links = [
    ...
    { path: "/phongkhach", title: "Giới thiệu" },
    { path: "/phongbep", title: "Sản phẩm" },
    { path: "/phonglamviec", title: "Tin tức" },
    { path: "/phongngu", title: "Liên hệ" },
  ];
  return (
            ...
            {links.map((link) => {
              return (
                <li key={link.path}>
                  {/* <NavLink
                    key={link.path}
                    className={clsx(styles.Footer1_link)}
                    to={link.path}
                  > */}
                  {link.title}
                  {/* </NavLink> */}
